...
def topological_transform(signals):
    ...
    steps = [("persistence", persistence), ("scaling", scaling), ("entropy", entropy)]
...
def calculate_entropies(full_dataset):
    ...
    for subject in full_dataset.keys():
        ...
        print(
            f"\tCompleted, took {(datetime.now() - start_time).total_seconds():.0f} seconds"
        )
    return entropies
...
def make_entropy_dataframe(entropies):
    return pd.DataFrame(
        entropies, columns=["subject", "H0 Entropy", "H1 Entropy", "H2 Entropy"]
    )
