...
def main():
    make_n_json_datasets(
        5, leg="right", location="thigh", num_periods=3, sample_size=100
    )
    make_n_json_datasets(
        5, leg="left", location="thigh", num_periods=3, sample_size=100
    )
...
def make_sub_dataset(
    subject, directory, leg="right", location="thigh", num_periods=4, sample_size=50
):
    ...
    sensor_location = {
        "right": {"foot": "rf", "shin": "rs", "thigh": "rt"},
    ...
    while len(signals) < sample_size:
        ...
        df_accel_loc = df_accel[
            [
                "acc_" + sensor_placed_where + "_x",
        ...
        if num_strides > num_periods:
            ...
            series = df_accel_loc[
                wind_start * period : wind_start * period + num_periods * period
            ]
...
def construct_dataset(
    directory="data/", leg="right", location="thigh", num_periods=4, sample_size=50
):
    full_dataset = {}

    for sub_num in range(1, 19):
        try:
            sub_dataset = make_sub_dataset(
                f"{sub_num}", directory, leg, location, num_periods, sample_size
            )
